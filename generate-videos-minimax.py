...
def generate_video_with_minimax(scene):
...
def create_final_video():
    ...
    for scene in SCENES:
        ...
        if scene['id'] < len(SCENES):
            # Create 8-second black video for spacing
            spacing_path = f"video-segments/spacing-{scene['id']:02d}.mp4"
            os.system(f"ffmpeg -f lavfi -i color=black:size=1280x720:duration=8 -y {spacing_path}")
            concat_content += f"file '{spacing_path}'\n"
            current_time += 8
    ...
    cmd = f"ffmpeg -f concat -safe 0 -i understand-me-video/concat-list.txt -c copy -y {final_output}"
    os.system(cmd)
    
    print(f"🎉 Final video created: {final_output}")
    print(f"⏱️  Total duration: ~{current_time} seconds")
...
def main():
    ...
    generated_videos = []
    for scene in SCENES:
        video_path = generate_video_with_minimax(scene)
        if video_path:
