/*
  # Initial Schema for understand.me

  1. New Tables
    - `profiles` - User profiles and preferences
    - `conflict_sessions` - Conflict resolution sessions
    - `conversation_messages` - Messages in conversations
    - `emotion_analyses` - Emotional analysis data
    - `ai_insights` - AI-generated insights
    - `session_notes` - Notes for sessions
    - `participant_invitations` - Invitations to sessions
    - `message_reactions` - Reactions to messages
    - `session_participants` - Active session participants
    - `user_analytics` - User progress and analytics
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'conflict_sessions') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'emotion_analyses') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'session_notes') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'participant_invitations') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'session_participants') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_analytics') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_conflict_sessions_creator') THEN
    CREATE INDEX idx_conflict_sessions_creator ON conflict_sessions(creator_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_conflict_sessions_status') THEN
    CREATE INDEX idx_conflict_sessions_status ON conflict_sessions(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_conflict_sessions_type') THEN
    CREATE INDEX idx_conflict_sessions_type ON conflict_sessions(session_type);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_conversation_messages_session') THEN
    CREATE INDEX idx_conversation_messages_session ON conversation_messages(session_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_conversation_messages_sender') THEN
    CREATE INDEX idx_conversation_messages_sender ON conversation_messages(sender_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_conversation_messages_created') THEN
    CREATE INDEX idx_conversation_messages_created ON conversation_messages(created_at);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_emotion_analyses_session') THEN
    CREATE INDEX idx_emotion_analyses_session ON emotion_analyses(session_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_emotion_analyses_user') THEN
    CREATE INDEX idx_emotion_analyses_user ON emotion_analyses(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_emotion_analyses_created') THEN
    CREATE INDEX idx_emotion_analyses_created ON emotion_analyses(created_at);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_ai_insights_session') THEN
    CREATE INDEX idx_ai_insights_session ON ai_insights(session_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_ai_insights_user') THEN
    CREATE INDEX idx_ai_insights_user ON ai_insights(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_ai_insights_type') THEN
    CREATE INDEX idx_ai_insights_type ON ai_insights(insight_type);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_analytics_user') THEN
    CREATE INDEX idx_user_analytics_user ON user_analytics(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_session_notes_session') THEN
    CREATE INDEX idx_session_notes_session ON session_notes(session_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_session_notes_author') THEN
    CREATE INDEX idx_session_notes_author ON session_notes(author_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_session_notes_category') THEN
    CREATE INDEX idx_session_notes_category ON session_notes(category);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_session_notes_created') THEN
    CREATE INDEX idx_session_notes_created ON session_notes(created_at);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_participant_invitations_session') THEN
    CREATE INDEX idx_participant_invitations_session ON participant_invitations(session_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_participant_invitations_inviter') THEN
    CREATE INDEX idx_participant_invitations_inviter ON participant_invitations(inviter_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_participant_invitations_status') THEN
    CREATE INDEX idx_participant_invitations_status ON participant_invitations(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_participant_invitations_token') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_session_participants_status') THEN
...
-- Apply updated_at triggers (will silently fail if they already exist)
DROP TRIGGER IF EXISTS update_profiles_updated_at ON profiles;
CREATE TRIGGER update_profiles_updated_at BEFORE UPDATE ON profiles FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_conflict_sessions_updated_at ON conflict_sessions;
...
BEGIN
  ...
  EXECUTE 'ALTER TABLE IF EXISTS conflict_sessions ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS conversation_messages ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS emotion_analyses ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS ai_insights ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS user_analytics ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS session_notes ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS participant_invitations ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS message_reactions ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE IF EXISTS session_participants ENABLE ROW LEVEL SECURITY';
  ...
  DROP POLICY IF EXISTS "Users can view own sessions" ON conflict_sessions;
  DROP POLICY IF EXISTS "Users can create sessions" ON conflict_sessions;
  DROP POLICY IF EXISTS "Users can update own sessions" ON conflict_sessions;
  
  -- Conversation messages policies
  DROP POLICY IF EXISTS "Users can view session messages" ON conversation_messages;
  DROP POLICY IF EXISTS "Users can insert messages" ON conversation_messages;
  
  -- Emotion analyses policies
  DROP POLICY IF EXISTS "Users can view own analyses" ON emotion_analyses;
  DROP POLICY IF EXISTS "Users can insert own analyses" ON emotion_analyses;
  ...
  DROP POLICY IF EXISTS "Users can view session notes" ON session_notes;
  DROP POLICY IF EXISTS "Users can create session notes" ON session_notes;
  ...
  DROP POLICY IF EXISTS "Users can view session invitations" ON participant_invitations;
  DROP POLICY IF EXISTS "Users can create invitations" ON participant_invitations;
  DROP POLICY IF EXISTS "Users can update own invitations" ON participant_invitations;
  
  -- Message reactions policies
  DROP POLICY IF EXISTS "Users can view message reactions" ON message_reactions;
  DROP POLICY IF EXISTS "Users can create own reactions" ON message_reactions;
...
-- Create views for dashboard and analytics (only if they don't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_views WHERE viewname = 'user_dashboard_stats') THEN
    EXECUTE '
  ...
  IF NOT EXISTS (SELECT FROM pg_views WHERE viewname = 'session_analytics') THEN
    EXECUTE '
...
-- Create database functions (only if they don't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_user_stats') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_session_analytics') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'calculate_personality_match') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_session_notes') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'generate_invitation_token') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_message_reactions') THEN
