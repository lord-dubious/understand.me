...
### 6.1. Onboarding Process
    ...
    *   **6.1.4. Core Logic & Workflow:**
        *   **User Sign-Up/Login Logic (UI Guide 2.2):**
            ...
            4.  **Expo App (State Update):** Securely stores the session (handled by Supabase JS client in AsyncStorage) and updates global state (e.g., Zustand `userStore` - Dev Guide 7.3) with user profile and authentication status. User profile data might be fetched immediately after login and potentially cached using Upstash Redis (via a Supabase Edge Function or PicaOS) for quick access upon subsequent app opens, especially if the profile data grows extensive.
            5.  **Navigation:** Navigates to Personality Assessment (for new users) or Main Dashboard.
            6.  **PicaOS/Dappier Role (Optional):**
                *   If Dappier is used for Decentralized Identity (DID): The social login or sign-up process might involve PicaOS orchestrating calls to Dappier to register/authenticate the user's DID and link it to their Supabase Auth ID. This would be a more complex flow involving redirects or additional steps.
                *   PicaOS might orchestrate a post-signup welcome flow (e.g., sending a welcome email via a Supabase Edge Function, or scheduling a "check-in" notification via Nodely).
        ...
        *   **Interactive Platform Tutorial Logic (UI Guide 2.4):**
            ...
            2.  **Tutorial Configuration (Expo App/PicaOS):** The structure of the tutorial (steps, content, highlighted UI elements) could be:
                *   Statically defined within the Expo app.
                *   Fetched from Supabase (e.g., a `tutorial_steps` table) if it needs to be dynamically updated without an app release. This fetched configuration could be cached in Upstash Redis (via PicaOS or an Edge Function) for faster loads.
            3.  **Step Progression (Expo App):** User interacts with UI elements as guided by Alex (Component 10.2) and tutorial overlays (React Native components). Each step completion updates local state.
            ...
            6.  **PicaOS Role (Optional):** PicaOS might be called by the tutorial if any step requires AI interaction or analysis (e.g., "Try asking Alex a question about X").
    *   **6.1.5. Frontend Implementation (Expo/React Native):**
        ...
        *   **Tutorial:** Use a state machine or step counter for tutorial progress. UI overlays using absolutely positioned `<View>`s or a library like `react-native-copilot`. Alex's voice/text via Component 10.2.
    *   **6.1.6. Backend/Service Interactions:**
        ...
        *   **PicaOS/Dappier (Optional for Sign-Up/Login):** Advanced identity solutions or post-signup flows.
    ...
    *   **6.1.7. Data Flow Diagram (Conceptual - Sign-Up):**
        ...
        ExpoApp (Credentials) -> Supabase Auth (Create User & Session) -> Supabase DB (Trigger: Create Profile)
                                                                     ...
                                                                     -> (Optional) PicaOS (Post-Signup Welcome Flow)
        ```
...
### 6.2. Host Path - Session Creation & Setup
    ...
    *   **6.2.4. Core Logic & Workflow:**
        ...
        *   **6.2.B. AI Problem Analysis Review (UI Guide 4.2):**
            1.  **Trigger (Expo App):** After host submits conflict description (6.2.A), PicaOS returns a `session_id` and confirmation that analysis is in progress. App navigates to this screen, polling PicaOS (e.g., `/session/{sessionId}/analysis_status`) or listening to a Supabase Realtime event orchestrated by PicaOS for analysis completion.
            2.  **Fetch Analysis (Expo App):** Once PicaOS indicates completion, Expo app fetches structured analysis results from PicaOS (e.g., `/session/{sessionId}/analysis_results`). This data might be cached in Upstash Redis by PicaOS for quick retrieval if the analysis is extensive or frequently accessed before session start.
            3.  **Display (Expo App):** Renders themes, sentiments, divergences, talking points using `<Text>`, cards (`<View>`), simple charts (`react-native-svg-charts`). Allows user feedback (ratings/comments on insights) via Component 10.8, saved to Supabase via PicaOS or direct Supabase call.
            4.  **User Edits/Confirmations (Expo App):** Host can edit/rephrase/dismiss AI insights. Changes are sent back to PicaOS to update the session's AI analysis data in Supabase (e.g., in `sessions.ai_synthesis_summary` or a related table).
            5.  **PicaOS/Nodely Role:** PicaOS handles the orchestration of fetching file content (potentially from Nodely/IPFS if files were pinned early by an advanced PicaOS pre-processing step) and sending it to Google GenAI for analysis. Results are stored in Supabase.
        *   **6.2.C. Configure Session Type (UI Guide 4.3):**
            1.  **Load Configuration Options (Expo App):** Session type templates (names, descriptions, default settings) might be fetched from Supabase (could be cached in Upstash Redis via PicaOS/Edge Function) or be part of app config. AI recommendations for session types (based on analysis from 6.2.B) are fetched from PicaOS.
            ...
            3.  **Persist Configuration (Expo App):** Selected configuration (session type, duration, enabled features like transcription/translation, Q&A, polls, anonymity) is saved to the `sessions` table in Supabase for the current `session_id` (likely via a PicaOS endpoint like `/session/{sessionId}/configure`).
            4.  **PicaOS Role:** If session setup involves complex orchestration (e.g., provisioning specific AI models or Dappier data feeds based on session type), PicaOS handles this based on the configuration received.
        *   **6.2.D. Add Participants & Send Invitations (Joint Remote) (UI Guide 4.4):**
            ...
            2.  **Store Participants (Expo App to Supabase/PicaOS):** For each participant, a record is created in `session_participants` linked to the `session_id`, including their email and role. This can be done via a PicaOS endpoint that then writes to Supabase.
            3.  **Customize Invitation (Expo App):** Host edits message in a Modal (Component 10.7).
            4.  **Dispatch Invitations (PicaOS/Supabase Function):** Expo app calls a PicaOS endpoint (e.g., `/session/{sessionId}/send_invitations`). PicaOS then uses a Supabase Edge Function or its own logic (possibly with Nodely for email template management or dispatch) to send emails (via Supabase Auth or third-party email service) and/or in-app notifications (via Supabase Realtime/`expo-notifications`). Unique invitation links/tokens are generated.
        *   **6.2.E. Track Invitation Status (Joint Remote) (UI Guide 4.5):**
            1.  **Fetch Status (Expo App):** Retrieves participant list and their `invitation_status` from `session_participants` for the current `session_id` via Supabase query (direct or through PicaOS).
            ...
            3.  **Resend/Edit Invitation (Expo App & PicaOS/Supabase Function):** Host actions trigger calls to PicaOS/Supabase Function to resend email or update participant details.
    ...
    *   **6.2.6. Backend/Service Interactions:**
        *   **PicaOS:** Orchestrates AI analysis (Google GenAI), manages session configuration logic, participant addition, and triggers invitation dispatch.
        ...
        *   **Upstash Redis (via PicaOS/Edge Function):** Caching AI analysis results, session type templates.
    *   **6.2.7. Data Flow Diagram (Conceptual - Configure Session Type):**
        ```
        ExpoApp (SessionTypeSelection, Settings) -> PicaOS (/session/{id}/configure)
                                                    -> Supabase DB (Update 'sessions' table)
                                                    -> (If complex) PicaOS orchestrates Dappier/other service setup
                                                    -> PicaOS -> ExpoApp (Confirmation)
        ```
    *   **6.2.8. Error Handling:**
        *   Expo App: Handle API errors from PicaOS/Supabase. Provide user feedback (Toasts - Component 10.6). Validate forms.
        *   PicaOS/Supabase Functions: Robust error handling for calls to GenAI, DB updates, invitation dispatch. Log errors to Sentry.
...
### 6.3. Participant Path - Joining Session
    ...
    *   **6.3.4. Core Logic & Workflow:**
        *   **6.3.A. Enter Session Code (UI Guide 5.1):**
            ...
            2.  **Validation Request (Expo App):** App calls a PicaOS endpoint (e.g., `/session/validate_code`) or a Supabase Edge Function.
                *   **Caching/Rate Limiting (Upstash Redis):** The backend (PicaOS/Edge Function) might use Upstash Redis to cache valid session codes for quick lookups or to implement rate limiting on code validation attempts to prevent abuse.
            3.  **Backend Logic (PicaOS/Edge Function):** Queries Supabase `sessions` table to find a session matching the code and status (e.g., 'scheduled', 'in_progress').
            4.  **Response (PicaOS/Edge Function to Expo App):** Returns session validity. If valid, returns `session_id` and details about next steps (e.g., if perspective sharing is required, if user is already a participant or needs to be added).
        ...
        *   **6.3.B. Receive Detailed Invitation (UI Guide 5.2):**
            1.  **Trigger (Expo App):** Participant opens a deep link (`understandmeapp://invitation/[inviteId]`). The `inviteId` could be a JWT or a unique token managed by Supabase/PicaOS. Dappier could be used if these tokens need to be verifiable or have specific decentralized properties.
            2.  **Fetch Invitation Details (Expo App):** App calls PicaOS (e.g., `/invitation/{inviteId}/details`) or a Supabase Edge Function.
            3.  **Backend Logic (PicaOS/Edge Function):** Validates `inviteId`. Fetches session details, host information, host's message, list of shared files (paths from Supabase Storage, or IPFS CIDs via Nodely if applicable) from Supabase (`sessions`, `profiles`, `session_files` tables).
                *   **Caching (Upstash Redis):** Frequently accessed invitation details (especially public session info or file metadata) could be cached by PicaOS/Edge Function to speed up loading.
            4.  **Response (PicaOS/Edge Function to Expo App):** Returns structured data for display.
        ...
        *   **6.3.C. Accept or Decline Invitation (UI Guide 5.3):**
            ...
            2.  **Update Status (Expo App):** App calls PicaOS (e.g., `/invitation/{inviteId}/respond`) or a Supabase Edge Function with the response (`accepted` or `declined`) and optional reason for declining (`<TextInput>`).
            3.  **Backend Logic (PicaOS/Edge Function):** Updates the `invitation_status` (and `reason_declined` if provided) in the `session_participants` table for that user and session in Supabase.
            4.  **Notifications (PicaOS/Supabase Function):** Can trigger a notification (email or in-app via Supabase Realtime/`expo-notifications`) to the Host about the participant's response. Nodely could be used for complex notification workflows.
        ...
        *   **6.3.D. Provide Your Perspective (UI Guide 5.4):**
            ...
            2.  **Submit Perspective (Expo App):** Sends text and file references (storage paths) to PicaOS (e.g., `/session/{sessionId}/participant_perspective`) or Supabase Edge Function.
            3.  **Backend Logic (PicaOS/Edge Function):** Saves `perspective_text` and links uploaded files (new records in `session_files` with `uploader_profile_id` as participant) to the `session_participants` record in Supabase.
            4.  **AI Analysis (Optional, by PicaOS):** PicaOS *may* trigger an asynchronous light analysis of the participant's input using Google GenAI for early insight generation for the host (results stored in a separate table or appended to participant's perspective).
        ...
        *   **6.3.E. Configure Privacy Settings (UI Guide 5.5):**
            ...
            2.  **Save Settings (Expo App):** User selections are saved back to their `profiles` table in Supabase (e.g., `profile_visibility`, `allow_growth_analysis`, etc.) via a direct Supabase client call from `profileService.ts` or a PicaOS endpoint.
                *   **Dappier Role (Optional):** If privacy settings involve granular, verifiable consent for data usage across different services (e.g., specific AI analyses by PicaOS, data sharing with Dappier-connected services), Dappier could be used to manage these consents as verifiable credentials.
    ...
    *   **6.3.5. Frontend Implementation (Expo/React Native):**
        ...
        *   Services for PicaOS/Supabase calls. State management (Zustand) for user inputs, session details.
    *   **6.3.6. Backend/Service Interactions:**
        *   **PicaOS/Supabase Edge Functions:** Validate session codes, fetch invitation details, update invitation status, store perspectives, trigger notifications.
        ...
        *   **Google GenAI (via PicaOS):** Optional analysis of participant perspectives.
    ...
    *   **6.3.7. Data Flow Diagram (Conceptual - Accept Invitation):**
        ```
        ExpoApp (Acceptance) -> PicaOS (/invitation/{id}/respond)
                                -> Supabase DB (Update 'session_participants')
                                -> PicaOS (Trigger Host Notification via Supabase Realtime/Email)
                                -> PicaOS -> ExpoApp (Confirmation, Nav to Next Step)
        ```
    *   **6.3.8. Error Handling:**
        *   Expo App: Handle invalid session codes, failed API calls to PicaOS/Supabase, file upload errors. Display user-friendly errors (Toasts - Component 10.6).
...
### 6.4. Pre-Session Preparation (Converged Path)
    ...
    *   **6.4.4. Core Logic & Workflow:**
        *   **6.4.A. AI Synthesizes All Inputs & Dynamic Adaptation (UI Guide 6.1):**
            ...
            2.  **Request to PicaOS (Expo App):** App sends `session_id` to PicaOS (e.g., `/session/{sessionId}/synthesize_inputs`).
            3.  **PicaOS Orchestration:**
                ...
                *   If Dappier is used for RAG, PicaOS might query Dappier for additional context based on initial inputs.
            ...
            4.  **Google GenAI Processing:** Analyzes the combined input and returns structured synthesis to PicaOS.
            5.  **PicaOS Stores & Caches Synthesis:** PicaOS stores this synthesized overview in Supabase (e.g., `sessions.ai_synthesis_summary` or a dedicated table). For faster subsequent access by the host before session start, this synthesis could be cached in Upstash Redis by PicaOS.
            6.  **Response to Expo App:** PicaOS returns the synthesized information (themes, sentiments, divergences, suggested adaptations) to the Expo app.
        ...
        *   **6.4.B. Establish Session Goals & Rules (UI Guide 6.2):**
            1.  **Load Suggestions (Expo App):** App requests suggested goals/rules from PicaOS (e.g., `/session/{sessionId}/suggest_goals_rules`), which uses the synthesis (6.4.A) and session type to generate them via Google GenAI. Alternatively, templates from Supabase (cached in Upstash Redis) could be used.
            ...
            3.  **Save to Supabase (Expo App):** Finalized goals and rules are saved to the `sessions` table (JSONB fields `goals`, `rules`) for the `session_id`, likely via a PicaOS endpoint (`/session/{sessionId}/finalize_goals_rules`).
            4.  **Optional Sharing (PicaOS/Supabase Function):** If host chooses to share with participants pre-session, PicaOS/Supabase Function could trigger notifications (Component 10.6) or update a shared resource.
        *   **6.4.C. Same-Device Setup (UI Guide 6.3 - Re-detailing Logic):**
            ...
            4.  **Sequential Mini-Assessment (Expo App & PicaOS - UI Guide 2.3 logic):**
                *   For each newly named participant on the device who doesn't have `profiles.preferred_communication_style` data:
                    ...
                    *   Expo app sends response to PicaOS (e.g., `/assessment/mini_submit`).
                    *   PicaOS -> Google GenAI (for minimal analysis if needed) -> PicaOS.
                    *   PicaOS saves this limited assessment data to the participant's `profiles` record in Supabase (or a temporary store if they are not existing users).
            5.  **Tap-to-Talk Training (Expo App - Component 10.4):**
                *   Alex (Component 10.2) explains the tap-to-talk mechanism.
    ...
    *   **6.4.5. Frontend Implementation (Expo/React Native):**
        ...
        *   API service calls to PicaOS.
    *   **6.4.6. Backend/Service Interactions:**
        *   **PicaOS:** Fetches all inputs from Supabase, orchestrates Google GenAI for synthesis & goal/rule suggestions, saves finalized goals/rules to Supabase, handles mini-assessments.
        ...
        *   **Upstash Redis (via PicaOS/Edge Function):** Caching synthesized overviews, session type templates, goal/rule templates.
        *   **Dappier (Optional):** Could provide external data to enrich PicaOS's synthesis process if needed.
    *   **6.4.7. Data Flow Diagram (Conceptual - AI Synthesis):**
        ```
        ExpoApp (Host requests synthesis for session_id) -> PicaOS (/session/{id}/synthesize_inputs)
            PicaOS -> Supabase DB (Fetch Host Input, Participant Perspectives, Profiles)
            PicaOS -> (Optional) Dappier (Fetch supplementary RAG data)
            PicaOS -> Google GenAI (Send all context for synthesis)
            Google GenAI -> PicaOS (Return structured synthesis)
            PicaOS -> Supabase DB (Store ai_synthesis_summary)
            PicaOS -> (Optional) Upstash Redis (Cache synthesis)
            PicaOS -> ExpoApp (Return synthesis to display)
        ```
    *   **6.4.8. Error Handling:**
        *   Expo App: Handle PicaOS API errors. Show Toasts (Component 10.6) or Modals (Component 10.7). Graceful degradation if synthesis fails partially.
        *   PicaOS: Robust error handling for Supabase, Google GenAI, Dappier, Redis calls. Log to Sentry. Return meaningful errors to app.
    *   **6.4.9. Code Snippets (Conceptual TypeScript):**
        ...
        // PicaOS: Conceptual - Orchestrating Synthesis
...
### 6.5. AI-Mediated Session Core (The Five Phases)
    *   **6.5.1. Overview & Goal:** To implement the five-phase AI-mediated session interface, providing a structured, guided, and interactive experience for all participants. This involves real-time transcription, AI-driven guidance from Alex, turn management, and tools for understanding and resolution.
    ...
    *   **6.5.4. Core Logic & Workflow:**
        *   **6.5.A. Common Session Interface Logic (UI Guide 7.1):**
            1.  **Overall Session State Management (Expo App):** Use Zustand store (`sessionStore`) to manage:
                ...
                *   Session goals, rules, AI-generated insights from PicaOS.
                *   Real-time messages/transcript entries.
                *   List of participants and their statuses (e.g., connected, speaking, muted).
                *   This state can be cached in Upstash Redis by PicaOS for quick rehydration if a user briefly disconnects and reconnects, or if PicaOS needs to maintain state across multiple serverless function invocations for that session.
            2.  **Message/Transcript Handling (Expo App & Supabase Realtime):**
                ...
                *   New messages (user speech transcripts from PicaOS/Google STT, user typed messages, Alex's guidance text) are inserted into `session_messages` by PicaOS or directly by app (for typed messages), and arrive in real-time.
                *   Display messages in a `<FlatList>` (Component 7.1.C).
            3.  **Alex's Guidance Display (Expo App):**
                *   Alex's text scripts (received from PicaOS) are displayed in the AI Panel (Component 7.1.B / 10.2).
                *   Alex's voice (audio URL from PicaOS via ElevenLabs) is played using `expo-av` (Component 10.2).
            ...
            5.  **Input Area (Expo App - Component 7.1.E, 10.1, 10.8):** Handles text input, voice input (sending audio to PicaOS for STT/processing), and file attachments (uploading to Supabase Storage, informing PicaOS).
            6.  **Same-Device Logic (Expo App - Component 7.1.F, 10.4):** Manages local UI state for tap-to-talk, active speaker indication on the shared device. Sends identified speaker's contributions to PicaOS.
            7.  **PicaOS as Session Conductor:** PicaOS is the central orchestrator. It receives all significant events/inputs from the app (user speech, typed messages, phase progression requests), maintains the canonical AI-driven session state, interacts with Google GenAI for analysis/scripting, and sends directives back to the app.
        *   **6.5.B. Phase 1: Prepare (UI Guide 7.2):**
            1.  **Initiation (PicaOS & Expo App):** PicaOS signals start of phase. Expo app displays goals/rules (fetched from `sessions` table via PicaOS/Supabase). Alex provides welcome and instructions (Component 10.2).
            2.  **Readiness Check (Expo App & PicaOS):** Participants tap "Ready" (`<TouchableOpacity>`). App sends readiness status to PicaOS. PicaOS updates `session_participants.status` in Supabase and determines when all are ready (or host overrides).
        *   **6.5.C. Phase 2: Express (UI Guide 7.3):**
            1.  **Turn Management (PicaOS):** PicaOS defines speaking order (e.g., based on pre-session input order, host setting, or round-robin). Communicates current speaker to Expo app.
            2.  **User Input (Expo App):** Active speaker uses voice (Component 10.1 sending audio to PicaOS) or text (Component 10.8).
            3.  **PicaOS Orchestration:**
                ...
                *   (Optional) Light analysis by Google GenAI for immediate sentiment cue (Component 10.5) or keyword spotting if needed by PicaOS for phase logic.
            4.  **Same-Device Management (Expo App - Component 10.4):** UI manages tap-to-talk; only active speaker's audio/text is sent to PicaOS with their `profile_id`.
        *   **6.5.D. Phase 3: Understand (UI Guide 7.4):**
            1.  **Synthesis by PicaOS:** PicaOS fetches all "Express" phase messages from Supabase. Uses Google GenAI to summarize each participant's points, identify themes, alignments, divergences. May use Dappier for RAG if external context is needed to understand a point.
            2.  **Display Insights (Expo App):** Shows summaries, maps (if applicable, e.g., `react-native-svg` or static `<Image>`), and clarification prompts from Alex (scripted by PicaOS/GenAI) in AI Panel.
            3.  **Clarification Loop (Expo App & PicaOS):** Users ask questions (voice/text). PicaOS routes questions, gets answers (from users or by prompting GenAI for rephrasing), and updates UI.
            4.  **Caching (Upstash Redis):** PicaOS can cache the generated summaries/maps for the session in Upstash Redis to avoid re-computation if the phase is revisited or if there are many participants.
        *   **6.5.E. Phase 4: Resolve (UI Guide 7.5):**
            1.  **Brainstorming (PicaOS & Expo App):** Alex (scripted by PicaOS/GenAI) prompts for solutions. Users submit ideas (voice/text). PicaOS lists ideas in AI Panel via app state.
            2.  **Evaluation (PicaOS & Expo App):** Alex guides discussion on pros/cons. PicaOS may use GenAI to summarize arguments for/against options.
            3.  **Agreement Tracking (Expo App & PicaOS):** Users indicate agreement (e.g., via `<TouchableOpacity>` polls sent to PicaOS). PicaOS tracks consensus, updates AI Panel. Agreed solutions/actions saved by PicaOS to `sessions` or a dedicated `session_resolutions` table in Supabase.
            4.  **Dappier Role (Optional):** If agreements need to be verifiable or trigger off-chain actions, PicaOS could interact with Dappier to record them.
        *   **6.5.F. Phase 5: Heal (UI Guide 7.6):**
            1.  **Reflection Prompts (PicaOS & Expo App):** Alex (scripted by PicaOS/GenAI) provides prompts.
            2.  **Capture Reflections (Expo App):** Users share (voice/text, public/private). Public reflections go to `session_messages`. Private ones might go to a `user_reflections` table in Supabase, linked to user & session, via PicaOS.
            3.  **Closing (PicaOS & Expo App):** Alex delivers closing remarks. PicaOS finalizes session state in Supabase (e.g., `sessions.status = 'completed'`, `actual_end_time`).
    *   **6.5.5. Frontend Implementation (Expo/React Native):**
        ...
        *   Realtime subscriptions to Supabase for messages, participant status. API calls to PicaOS for AI logic and state progression.
    *   **6.5.6. Backend/Service Interactions:**
        *   **PicaOS:** Core orchestrator for all AI logic, session state, phase transitions, calls to Google GenAI, ElevenLabs, Dappier.
        ...
        *   **Google GenAI:** STT, core LLM for analysis, summarization, Alex's script generation.
        *   **ElevenLabs:** TTS for Alex.
        *   **Dappier (Optional):** RAG, verifiable agreements.
        *   **Nodely (Optional):** Accessing files from IPFS if shared during session.
        *   **Upstash Redis (via PicaOS):** Caching session state, frequently used prompts, AI-generated summaries for performance.
    *   **6.5.7. Data Flow Diagram (Conceptual - User Expresses, Alex Responds):**
        ```
        ExpoApp (User Audio/Text) -> PicaOS (STT if Audio)
                                    -> Supabase DB (Store user message in session_messages)
                                    -> PicaOS (Contextualize for Alex) -> Google GenAI (Generate Alex script)
                                    -> PicaOS -> ElevenLabs (Synthesize voice)
                                    -> PicaOS -> Supabase DB (Store Alex message in session_messages)
                                    -> PicaOS -> ExpoApp (via Supabase Realtime or direct push: Alex script & voice URL)
                                    -> ExpoApp (Display Alex Text & Play Voice)
        ```
    *   **6.5.8. Error Handling:**
        *   Expo App: Handle PicaOS API errors, Supabase Realtime connection issues, media playback errors. Display Toasts or Modals (Component 10.6, 10.7). Offer options to retry or inform user.
        *   PicaOS: Robust error handling for all external service calls (GenAI, ElevenLabs, Dappier, Supabase, Redis). Implement fallbacks (e.g., Alex text-only if ElevenLabs fails). Log detailed errors to Sentry with correlation IDs.
    *   **6.5.9. Code Snippets (Conceptual TypeScript):**
        ...
        //   // Call PicaOS to process and store
        ...
        // PicaOS: Conceptual - Processing a user message and getting Alex's response
        ...
        //   // 3. const prompt = createAlexResponsePrompt(sessionState, userContent, ...);
        ...
        //   // 6. Store Alex's response in Supabase session_messages
        //   // 7. (PicaOS might push to Supabase Realtime, or Expo app gets it via its own subscription)
        //   // return { alexScript: alexTextResponse, alexVoiceUrl }; // Or PicaOS pushes this to app via a websocket/realtime channel if not using Supabase Realtime for Alex's messages
        // }
        ```
...
### 6.6. Post-Session Activities
    ...
    *   **6.6.4. Core Logic & Workflow:**
        *   **6.6.A. AI Generates Summary & Action Plan (UI Guide 8.1):**
            1.  **Trigger (PicaOS):** After Phase 5 (Heal) concludes, or Host manually triggers "End Session & Generate Summary".
            2.  **PicaOS Orchestration:**
            ...
            3.  **PicaOS Stores Summary:** Saves the AI-generated summary, decisions, and action plan into the `sessions` table (e.g., `sessions.generated_summary_json`) or a dedicated `session_summaries` table in Supabase.
            4.  **Caching (Upstash Redis via PicaOS - Optional):** PicaOS might cache the generated summary for a short period for quick initial access by the host/participants.
            5.  **Notification (PicaOS to Expo App):** PicaOS informs the Expo app (via Supabase Realtime or a direct response if synchronous) that the summary is ready.
        ...
        *   **6.6.B. Participants Review & Approve Summary (UI Guide 8.2):**
            1.  **Host Initiates Review (Expo App):** Host, after reviewing the AI summary (and potentially making minor edits via a PicaOS endpoint), triggers the participant review process.
            2.  **PicaOS/Supabase Function:** Updates session status in Supabase (e.g., `sessions.status = 'pending_approval'`). Sends notifications (Component 10.6 via `expo-notifications` / email) to participants.
            3.  **Participant Review (Expo App):**
                ...
                *   Participant clicks "Approve" or "Request Changes". This updates their status in `session_participants.summary_approval_status` via a PicaOS call or direct Supabase update.
            4.  **Host View (Expo App):** Host sees approval statuses. If changes requested, Host can create a new version of the summary (managed by PicaOS/Supabase).
        *   **6.6.C. Digital Sign-off (UI Guide 8.3):**
            ...
            3.  **Record Signature (Expo App to PicaOS/Supabase Function):**
                *   A record of the sign-off (user_id, session_id, timestamp, signature_data - which might be the typed name or a cryptographic hash if more formal) is sent to PicaOS or a Supabase Edge Function.
                *   **Dappier Role (Optional):** For verifiable signatures, PicaOS/Edge Function could interact with Dappier to create a verifiable credential or attestation for the sign-off, linking it to the user's DID.
        ...
        *   **6.6.D. Session Evaluation & Feedback (UI Guide 8.4):**
            ...
            2.  **Submit Feedback (Expo App):** User fills out ratings (`react-native-ratings`) and text feedback (`<TextInput>`). Data sent to PicaOS or directly to Supabase.
        ...
        *   **6.6.E. Schedule Follow-up Check-ins (UI Guide 8.5):**
            ...
            2.  **Check Availability (Expo App & PicaOS/Supabase Function - Optional):** If `expo-calendar` access is granted, app can send availability info to PicaOS/Supabase Function to suggest common times.
            3.  **Create Follow-up Session (Expo App & PicaOS/Supabase Function):** Details (title, purpose, participants, time) sent to PicaOS/Supabase Function. A new `sessions` record is created in Supabase with a link to the original session.
            4.  **Send Invitations (PicaOS/Supabase Function):** Triggers email/in-app notifications (Component 10.6) for the follow-up session. Calendar invites created using `expo-calendar` on client, or sent via backend email service.
    ...
    *   **6.6.6. Backend/Service Interactions:**
        *   **PicaOS:** Orchestrates summary generation (Google GenAI), manages review/approval state changes, triggers notifications, handles scheduling logic.
        ...
        *   **Upstash Redis (via PicaOS/Edge Function):** Caching generated summaries before review.
    *   **6.6.7. Data Flow Diagram (Conceptual - AI Summary Generation & Review Start):**
        ```
        PicaOS (End of Session Trigger) -> Supabase DB (Fetch session data)
                                        -> Google GenAI (Generate Summary)
                                        -> Supabase DB (Store Raw Summary)
                                        -> (Optional) Upstash Redis (Cache Summary)
                                        -> PicaOS -> Expo App (Host notified: Summary Ready)
        Expo App (Host reviews/edits, initiates participant review) -> PicaOS
                                        -> Supabase DB (Update session status to 'pending_approval')
                                        -> PicaOS (Trigger Notifications to Participants)
        ```
    *   **6.6.8. Error Handling:**
        *   Expo App: Handle API errors from PicaOS/Supabase. Graceful display if summary parts are missing. Clear feedback on submission status.
        *   PicaOS/Backend: Robust error handling for GenAI summary generation, DB updates, notification dispatch. Log to Sentry.
    *   **6.6.9. Code Snippets (Conceptual TypeScript):**
        ...
        // PicaOS/Supabase Edge Function: Digital Sign-off with Dappier (Conceptual)
...
*   **6.2.A.8. Error Handling:**
    *   Expo App: Handle file picker errors, upload errors (Supabase Storage), PicaOS API errors. Display Toasts or Modals (Component 10.6, 10.7).
    *   PicaOS: Handle Supabase DB errors, GenAI analysis errors. Return appropriate error codes. Log to Sentry.
*   **6.2.A.9. Code Snippets (Conceptual TypeScript):**
    ...
    // PicaOS: Conceptual logic (Illustrative)
